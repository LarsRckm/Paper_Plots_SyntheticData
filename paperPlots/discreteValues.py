...
from create_data import generate_noisy_data_slope, generate_noisy_data_periodic
    ...
    try:
        for i in range(10):
            ...
            if abbruch.lower() == 'q':
                ...
                ax1.plot(x_values, NoiseCopy,label= "MinMax(NoiseData)", color=colors["GT_N"], linewidth=linewidth_GT_N, alpha=alpha_GT_N)
                ax1.plot(x_values, SplineCopy, label="MinMax(GroundTruth)", color=colors["GT"],linewidth=linewidth_GT)
                
                ax2.plot(x_values, yNoiseRounded,label="Rounding(NoiseData)",color=colors["GT_N"],linewidth=linewidth_GT_N, alpha=alpha_GT_N)
                ax2.plot(x_values, ySplineRounded, label="Rounding(Groundtruth)", color=colors["GT"],linewidth=linewidth_GT)
