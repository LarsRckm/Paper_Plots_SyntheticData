...
def multipleSubplots():
    model_filename = [
        "weights/Encoder_Interpolation_CE_PeriodicSum_2000.pt", 
...
if __name__ == "__main__":
    #determine model
    model = "weights/Encoder_Interpolation_CE_PeriodicSum_2000.pt" 
    # model = "weights/Encoder_Interpolation_CE_Periodic_300.pt"
    # model = "weights/Encoder_Interpolation_CE_LowOrder_300.pt"
    # model = "weights/Encoder_Interpolation_CE_HighOrder_300.pt"
    # model = "weights/Encoder_Interpolation_CE_AllInOne_2400.pt"
    # model = "weights/Encoder_Interpolation_CE_Discontinuous_1200.pt"

    #create synthetic noisy Data function
    modelFilename_TimeseriesGenerator = {
            "_PeriodicSum_": [5],
            "_Periodic_": [3],
            "_LowOrder_": [0,1,2],
    ...
    #iterate through data generator to create synthetic data
    for key, value in modelFilename_TimeseriesGenerator.items():
            if key in model:
                #choose a arbitrary function generator
                arr_len = len(value)
                random_int = randint(0,arr_len-1)
                config = get_config()
                seq_len = 1000
    ...
    a4_width_inch = 6   #8.27  #DIN A4 Breite in Zoll für volle Breite
    a4_height_inch = 11  #DIN A4 Höhe in Zoll für volle Breite
    
    latex_font_size_pt = 15 # Schriftgröße in LaTeX-Punkten (z. B. 10pt, 12pt)
    points_to_inches = 1 / 72 # Umrechnung von Punkt in Zoll (1 pt = 1/72 Zoll)
    labelSizeY = latex_font_size_pt
    labelSizeX = latex_font_size_pt -3
    labelSizeLegend = latex_font_size_pt
    ...
    # Hauptfigur und äußeres GridSpec mit Abständen
    fig, (ax1, ax2) = plt.subplots(2,1, figsize=(a4_height_inch, a4_width_inch))

    #plot first subplot data
    for yValues, label, color in resultFirstPlot[0]:
        ax1.plot(x_values, yValues, label=f"{label}", linewidth=lineWidth, color=color)
    #plot second subplot data
    for yValues, label, color in resultSecondPlot[0]:
        maxValue = max(yValues)
        minValue = min(yValues)
        ax2.plot(x_values, yValues, label=f"{label}", linewidth=lineWidth, color=color)
    #display border to illustrate difference between prediction and groundTruth
    for lowerBorder, upperBorder, color in rangePlots:
        ax2.fill_between(x_values, lowerBorder, upperBorder, color=color, alpha=0.1)
    #display interpolation intervals in both subplots
    for bottom_border, upper_border in intervals[0]:
        ax1.axvspan(bottom_border - 0.5, upper_border + 0.5, color='red', alpha=0.1)
        ax2.axvspan(bottom_border - 0.5, upper_border + 0.5, color='red', alpha=0.1)
        

        
    ax1.set_ylabel(labels[0],fontsize=labelSizeY,fontweight='bold')
    ax2.set_ylabel(labels[1],fontsize=labelSizeY,fontweight='bold')
    ax2.set_xlabel("Time Steps",fontsize=labelSizeX)
    ax1.margins(x=0.0)
    ax2.margins(x=0.0)
    offset = (maxValue-minValue)*0.05
