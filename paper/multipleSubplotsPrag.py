...
def multipleSubplots():
    ...
    lineWidth = 2
    folderName = "MultipleSubplotsPrag"
    title = "MultipleSubplotsLO"
    fmt = "png"
    ...
    outer_grid = GridSpec(n_rows, 1, figure=fig)  # Abstände festlegen


    for idx in range(num_plots):
        # Inneres GridSpec für jeden Haupt-Subplot
        # inner_grid = GridSpecFromSubplotSpec(2, 1, subplot_spec=outer_grid[idx // 2, idx % 2])
        inner_grid = GridSpecFromSubplotSpec(2, 1, subplot_spec=outer_grid[idx])  # 2x1 Inneres Layout
        ...
        if(idx == 0):
            ax1.legend(loc='best',prop={'size': labelSizeLegend})
