...
if __name__ == "__main__":
    noisyDataColor = "blue"
    groundTruthColor = "orange"
    predictionColors = ["purple", "mediumvioletred", "darkturquoise", "yellowgreen"]
    rangeColors = ["navy", "royalblue", "lightsteelblue"]
    ...
    result = []
    #first subplot
    result.append([[y_trend_noise, "Noisy Data", noisyDataColor], 
                   [y_trend, "GroundTruth", groundTruthColor], 
                   [prediction_encoder_LOWORDER, "LO", predictionColors[0]],[prediction_encoder_HIGHORDER, "HO", predictionColors[1]],[prediction_encoder_PERIODICSUM, "PS", predictionColors[2]],[prediction_encoder_ALLINONE, "C", predictionColors[3]]])
    #adding  the second subplots data
    result.append([
            [y_trend, "GroundTruth", "orange"], 
            [prediction_encoder_LOWORDER, "Prediction", predictionColors[0]],
            [prediction_encoder_HIGHORDER, "Prediction", predictionColors[1]],
            [prediction_encoder_PERIODICSUM, "Prediction", predictionColors[2]],
            [prediction_encoder_ALLINONE, "Prediction", predictionColors[3]]
        ])
    #adding the third subplots data
    result.append([[differenceLOWORDER, "relativer Fehler bei Sigma=1", predictionColors[0]],
                   [differenceHIGHORDER, "relativer Fehler bei Sigma=1", predictionColors[1]],[differencePERIODICSUM, "relativer Fehler bei Sigma=1", predictionColors[2]],
                   [differenceALLINONE, "relativer Fehler bei Sigma=1", predictionColors[3]]])
    
    
    #extra array storing the range plots
    resultRange= []
    #first plot does not contain range plots
    resultRange.append([])
    #second plot contains range plots based on std
    resultRange.append([[lower_deviation, upper_deviation, rangeColors[0]],
            [lower_deviation_2, upper_deviation_2, rangeColors[1]],
            [lower_deviation_3, upper_deviation_3, rangeColors[2]]])
    #third plot contains range plots based on std
    resultRange.append([[-1, 1, rangeColors[0]],
            [-2, 2, rangeColors[1]],
            [-3, 3, rangeColors[2]]])
    ...
    for i in range(subplot_count):
        ...
        if(i==2):
            ...
            ax.set_xlabel("Time Steps",fontsize=labelSize)
