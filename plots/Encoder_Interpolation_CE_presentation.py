...
from plot import predict_encoder_interpolation_projection_roundedInput,plot_multiple_pred_with_names_error_bar_single_presentation
from create_data import sliding_window
...
def plot_encoder_interpolation_CE():
    ...
    for i in range(10):
        prediction_encoder = sliding_window(prediction_encoder, 2)
    ...
    # result.append([[encoder_removed.detach().numpy(), "Noisy Data", "blue"], [y_values_spline, "Ground Truth", "orange"], [prediction_encoder.detach().numpy(), "Prediction", "green"]])
    ...
    result.append([
                   [y_masked, "Interpolation Data", "cornflowerblue"],
                    [prediction_encoder, "C", "yellowgreen"]
                    ])
    ...
    plot_multiple_pred_with_names_error_bar_single_presentation(x_values, result, titles, labels, intervalle)
