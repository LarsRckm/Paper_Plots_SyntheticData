...
from create_data import callFunction, remove_parts_of_graph_encoder
...
def plot_encoder_interpolation_CE():
    ...
    #--------------------------------------------------------------------------------
        ...
        a4_height_inch = 17  # für volle Breite
        ...
        lineWidth = 2
        _, axis = plt.subplots(3,1, sharex=True, figsize=(a4_height_inch, a4_width_inch))
        for i in range(3):
        ...
        for bottom_border, upper_border in intervalle:
            axis[0].axvspan(bottom_border - 0.5, upper_border + 0.5, color='red', alpha=0.1)
            axis[1].axvspan(bottom_border - 0.5, upper_border + 0.5, color='red', alpha=0.1)
            axis[2].axvspan(bottom_border - 0.5, upper_border + 0.5, color='red', alpha=0.1)




        #plot first subplot data
        axis[0].plot(x_values, y_masked, color="blue", linewidth = lineWidth, label="Noisy Data")
        axis[0].plot(x_values, y_values_spline, color="orange", linewidth = lineWidth, label="Groundtruth")
        axis[0].plot(x_values, prediction_encoder.detach().numpy(), color="green", linewidth = lineWidth, label="Prediction")

        #plot second subplot data
        axis[1].plot(x_values, y_values_spline, color="orange", linewidth = lineWidth)
        axis[1].plot(x_values, prediction_encoder.detach().numpy(), color="green", linewidth = lineWidth)
        axis[1].fill_between(x_values, lower_deviation, upper_deviation, color="navy", alpha=0.3)
        axis[1].fill_between(x_values, lower_deviation_2, upper_deviation_2, color="royalblue", alpha=0.3)
        axis[1].fill_between(x_values, lower_deviation_3, upper_deviation_3, color="lightsteelblue", alpha=0.3)

        #plot third subplot data
        maxdifferenceSTD = max(difference_std)
        mindifferenceSTD = min(difference_std)
        axis[2].plot(x_values, difference_std, color="navy", linewidth = lineWidth)
        axis[2].fill_between(x_values, -1,1, color="navy", alpha=0.3)
        axis[2].fill_between(x_values, -2,2, color="royalblue", alpha=0.3)
        axis[2].fill_between(x_values, -3,3, color="lightsteelblue", alpha=0.3)
        ...
        offset = (maxdifferenceSTD-mindifferenceSTD)*0.05
        axis[2].set_ylim(mindifferenceSTD-offset, maxdifferenceSTD+offset)
        plt.xlabel("Time Steps",fontsize=labelSizeX)

        save_path = Path(f"save_figure/explanation/")
        save_path.mkdir(parents=True, exist_ok=True)
        file_path = save_path / f"explanation1.svg" if save_path else f"explanation1.svg"
        plt.gcf().savefig(file_path, format="svg", dpi=300, bbox_inches='tight')
        plt.show()

        # plot_multiple_pred_with_names_error_bar_area(x_values, result, "explanation1", labels, "explanation/", intervalle)
